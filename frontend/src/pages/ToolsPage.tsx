...
import { exportAllData, importAllData, resetAllData } from '../data/store';
import {
  IconTools, IconDownload, IconUpload, IconTrash,
  IconPiggy, IconLayout, IconBook, IconWrench, IconClock,
...
const ToolsPage: React.FC = () => {
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (!file) return;
      try { const text = await file.text(); const ok = importAllData(text);
        if (ok) { setImportMsg({ type: 'success', text: '导入成功，页面刷新中' }); setTimeout(() => window.location.reload(), 1000); }
        else setImportMsg({ type: 'error', text: '数据格式不正确' });
      } catch { setImportMsg({ type: 'error', text: '文件读取失败' }); }
    };
    input.click();
  };

  const handleReset = () => { resetAllData(); setShowResetConfirm(false); setImportMsg({ type: 'success', text: '已重置，页面刷新中' }); setTimeout(() => window.location.reload(), 1000); };
    ...
    <AppShell currentPage="tools">
      <div className="tools-page">
        ...
        <div className="card" style={{ marginTop: 20 }}>
          ...
          <div className="card-bd">
            <p className="card-subtitle" style={{ marginBottom: 12 }}>所有数据保存在浏览器本地存储中。建议定期导出备份。</p>
