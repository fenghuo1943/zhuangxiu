...
export async function cleanupUploadedDiaryImages(urls: string[]): Promise<{ deleted_count: number; message: string }> {
  const headers: Record<string, string> = {
  ...
  const res = await fetch(`${API_BASE}/api/upload/cleanup`, {
    method: 'POST',
    headers,
